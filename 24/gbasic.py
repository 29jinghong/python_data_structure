...
#getting senders and id from senders database.
cur.execute('SELECT id, sender FROM Senders')
senders = dict()
for message_row in cur :
    senders[message_row[0]] = message_row[1]

#getting subject and id from subjects database.
cur.execute('SELECT id, subject FROM Subjects')
subjects = dict()
for message_row in cur :
    subjects[message_row[0]] = message_row[1]

#getting the id, guid, sender id, subject id, and send at from messages database.
...
sendcounts = dict()
sendorgs = dict()
#counts howmany messages each sender send
...
#prints the top 'howmany' email send by each email account.
...
#prints the list of the email organizations.
