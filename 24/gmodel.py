...
def fixsender(sender,allsenders=None) :
    ...
    if sender is None : 
        return None
    #fixes the sender if there is any '<' or '>', or any spaces and uppercase.
    ...
    if allsenders is not None and sender.endswith('gmane.org') :
        ...
        realsender = None
        for s in allsenders:
            #checks for the sender inside allsenders, and replace the old sender with the sender in allsender.
    ...
    #splits the sender into two parts and if there is not 2 parts it just takes that entire pice and treat is as the sender.
    mpieces = sender.split("@")
    if len(mpieces) != 2 : 
        return sender

    #if there is a sender like '29jinghong@gmail.com' it will take the 'gmail.com' and assine dns to it.     
    dns = mpieces[1]
    x = dns
    pieces = dns.split(".")
...
def parseheader(hdr, allsenders=None):
    ...
    #checks if any of the returned is none.
...
for message_row in cur_1 :
    ...
    #populating the allsenders, so it could be used later.
...
for message_row in cur_1 :
    ...
    #dont know why he added the None cause the difalt is none(which is if the .get dident find the value).
    sender_id = senders.get(sender,None)
    subject_id = subjects.get(subject,None)
    guid_id = guids.get(guid,None)

    #if cant find sender id, will look for sender id inside senders table
    ...
    #if cant find subject id, will look for subject id inside Subjects table
    ...
    #put the data into messages table with headers and body compressed by zlib.
